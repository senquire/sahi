...
def read_image_as_pil(images: List[Union[Image.Image, str, np.ndarray]], exif_fix: bool = False):
	"""
	...
	if not isinstance(images, list):
		return read_image_as_pil([images], exif_fix)
	for image in images:
			if isinstance(image, Image.Image):
				...
				try:
					...
					if len(image_sk.shape) == 2:  # b&w
						image_pil.append(Image.fromarray(image_sk, mode="1"))
					elif image_sk.shape[2] == 4:  # rgba
						image_pil.append(Image.fromarray(image_sk, mode="RGBA"))
					elif image_sk.shape[2] == 3:  # rgb
						image_pil.append(Image.fromarray(image_sk, mode="RGB"))
					else:
						raise TypeError(f"image with shape: {image_sk.shape[3]} is not supported.")
			elif isinstance(image, np.ndarray):
				if image.shape[0] < 5:	# image in CHW
					image = image[:, :, ::-1]
				image_pil.append(Image.fromarray(image))
			else:
